...
\begin{frame}
	\frametitle{OpenWSN}
	\begin{itemize}
		\item Provides open-source implementations of a complete protocol stack based on IoT standards, on a variety of software and hardware platforms.
...
\begin{frame}
	...
	\begin{itemize}
		...
		\begin{itemize}
			\item RIOT, Contiki, FreeRTOS, TinyOS, Linux
		\end{itemize}
	\end{itemize}
	\vspace{.5cm}
...
%------------------------------------------------
\begin{frame}
	\frametitle{The ``IoT-Lab" Experiments Steps}
	\vspace{.1cm}
	\begin{itemize}
		\item Build application
			\begin{itemize}
				\item Build one or several firmwares to use during experiment for WSN430, M3 or A8 hardware platform.			
			\end{itemize}
		\item Select resources
			\begin{itemize}
				\item Choose number and type of nodes or choose a specific topology.			
			\end{itemize}
		\item Configure nodes
			\begin{itemize}
				\item Associate a firmware to each node and define the activity of the control node during the experiment
			\end{itemize}
		\item Submit your experiment
			\begin{itemize}
				\item A scheduler will program experiment, immediately if resources are free. 
					\begin{itemize}
						\item[-] The deployment will be automatic.	
					\end{itemize}					 			
			\end{itemize}			
	\end{itemize}
\end{frame}
...
\begin{frame}
	\frametitle{IoT Toolkit}
	\begin{itemize}
		\item The IoT Toolkit is an Open Source project to develop a set of tools for building multi-protocol IoT Gateways and Service gateways that enable horizontal co-operation between multiple different protocols and cloud services.
	\end{itemize}
	\vspace{.1cm}
...
\begin{frame}
	...
	\begin{itemize}
		...
		\item There are many IDEs and platforms exist for developing IoT application, like:
		\begin{itemize}
			\justifying
			\item Eclipse IoT
			\item KinomaJS
			\item M2MLabs Mainspring
			\item Node-RED
		\end{itemize} 
	\end{itemize}
	\vspace{.5cm}
	\hspace*{3.5cm}
	\includegraphics[width=3cm]{figs/linux-logo.jpeg}
	\includegraphics[width=3cm]{figs/gun-logo.png}
\end{frame}
